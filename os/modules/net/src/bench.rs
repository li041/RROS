 ...
 * @Date: 2025-03-11 17:36:10
 * @LastEditors: Peter/peterluck2021@163.com
 * @LastEditTime: 2025-03-11 17:45:29
 * @FilePath: /net/src/bench.rs
 * @Description: 测试网络带宽，主要按照1GB大小发送MTU并计算其中的时间，和接受时间
 * 
...
use super::{DeviceWrapper, InterfaceWrapper, NetRxToken, NetTxToken, STANDARD_MTU};
...
impl DeviceWrapper {
    pub fn bench_send_bandwidth(&mut self) {
        // 10 Gb
        ...
        while send_bytes < MAX_SEND_BYTES {
            if let Some(tx_token) = self.transmit(InterfaceWrapper::current_time()) {
                //nettxtoken consume a send token to send a socket.
                //rycle send token
    ...
    pub fn bench_receive_bandwidth(&mut self) {
        ...
        while receive_bytes < MAX_RECEIVE_BYTES {
            if let Some(rx_token) = self.receive(InterfaceWrapper::current_time()) {
                //consume a token to receive a socket will 
                //rycle the receive tokens
                //alloc send token
                //exam send is ok
